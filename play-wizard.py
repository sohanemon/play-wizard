    ...
    for i in os.listdir(directory + "/paused-players/"):
        try:
            player = bus.get_object(
                "org.mpris.MediaPlayer2." + i, "/org/mpris/MediaPlayer2"
            )
        except:
...
def seek(offset):
    try:
        ...
        print(
            "Error: Offset must be an integer representing seconds (e.g., +10, -10, -30)."
        )
        return
...
def set_speed(speed):
    ...
    if relative:
        ...
        for i, initial_speed in initial_speeds.items():
            ...
            if new_speed <= 0:
                print(
                    f"Error: Resulting speed {new_speed} is invalid. Speed must be greater than 0."
                )
                continue
...
if len(sys.argv) - 1 >= 1:
    ...
    if sys.argv[1] in ["--version", "-v"]:
        ...
        print(
            "Error: Valid commands to "
            + sys.argv[0]
            + " are: pause, play, stop, next, previous, toggle, or seek <seconds>"
        )
else:
    print(
        "Usage: "
        + sys.argv[0]
        + " [pause|play|stop|next|previous|toggle|seek <seconds>]"
    )
